...
**File categorization rule:** `files_to_modify` is ONLY for files that already exist on disk; new files (including new docs / knowledge-base markdown) ALWAYS go in `files_to_create`. Mis-categorizing a not-yet-existing file as `files_to_modify` causes the build to fail validation and the subtask to be marked STUCK. When unsure, prefer `files_to_create`.
