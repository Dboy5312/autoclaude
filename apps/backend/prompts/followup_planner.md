...
`files_to_modify` and `files_to_create` are NOT interchangeable. The validator checks every entry in `files_to_modify` exists on disk before the coder runs. Mis-categorized entries cause STUCK subtasks.

- **`files_to_modify`** is ONLY for files that already exist (either in the original repo or created by an earlier completed subtask in this plan).
- **`files_to_create`** is for any file this follow-up introduces — including new docs, knowledge-base markdown, configs, fixtures, and test files.
- When unsure, prefer `files_to_create`.
