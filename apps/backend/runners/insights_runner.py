...
def build_system_prompt(project_dir: str) -> str:
    ...
    base = f"""You are an AI assistant helping developers understand and work with their codebase.
...
Keep responses concise but informative."""
    ...
    # Append user-configured chat preamble (output style + SOUL.md persona).
    # Sent from the Electron main process via AUTOCLAUDE_CHAT_PREAMBLE so the
    # insights chat respects per-user response shaping.
    preamble = os.environ.get("AUTOCLAUDE_CHAT_PREAMBLE", "").strip()
    if preamble:
        base = f"{base}\n\n{preamble}"
