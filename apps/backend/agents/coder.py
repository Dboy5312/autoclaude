...
def repair_subtask_files(
    subtask: dict,
    missing_files: list[str],
    spec_dir: Path,
) -> dict:
    """Reclassify missing files_to_modify entries as files_to_create.

    The planner sometimes mis-categorizes brand-new files (knowledge-base
    documents, new modules, new components) as files_to_modify. The validator
    then loops failing on the same missing paths until MAX_SUBTASK_RETRIES,
    even though the planner's intent was for these to be created.

    This repair moves the offending entries from files_to_modify to
    files_to_create both in the live subtask dict and in implementation_plan.json,
    so the next validation pass succeeds and the coder can create the files
    naturally as part of executing the subtask.

    Args:
        subtask: Subtask dict (mutated in place)
        missing_files: Paths from files_to_modify that don't exist on disk
        spec_dir: Spec directory containing implementation_plan.json

    Returns:
        dict with:
        - repaired (bool): True if the in-memory subtask was modified
        - persisted (bool): True if implementation_plan.json was rewritten
        - moved (list[str]): Files reclassified
        - error (str|None): Reason persistence failed, if applicable
    """
    if not missing_files:
        return {"repaired": False, "persisted": False, "moved": [], "error": None}

    files_to_modify = list(subtask.get("files_to_modify", []))
    files_to_create = list(subtask.get("files_to_create", []))
    moved: list[str] = []

    for path in missing_files:
        if path in files_to_modify:
            files_to_modify.remove(path)
        if path not in files_to_create:
            files_to_create.append(path)
        moved.append(path)

    subtask["files_to_modify"] = files_to_modify
    subtask["files_to_create"] = files_to_create
    ...
    if not plan_file.exists():
        return {
            ...
            "persisted": False,
            "moved": moved,
            "error": "implementation_plan.json not found; in-memory repair only",
        }
    ...
    try:
        ...
        return {
            ...
            "persisted": False,
            "moved": moved,
            "error": f"Could not read plan file: {e}",
        }
    ...
    if target is None:
        return {
            ...
            "persisted": False,
            "moved": moved,
            "error": f"Subtask {subtask_id!r} not found in plan; in-memory repair only",
        }

    target["files_to_modify"] = files_to_modify
    target["files_to_create"] = files_to_create
    ...
    try:
        ...
        return {
            ...
            "persisted": False,
            "moved": moved,
            "error": f"Could not persist plan: {e}",
        }

    return {"repaired": True, "persisted": True, "moved": moved, "error": None}
...
async def run_autonomous_agent(
    ...
    while True:
        ...
        if first_run:
            ...
            # Auto-repair: if validation failed only because files in files_to_modify
            # don't exist (no path-boundary violations), assume the planner
            # mis-categorized new files. Move them to files_to_create, persist the
            # plan, and re-validate. Avoids the previous infinite STUCK loop where
            # the same missing files would fail validation MAX_SUBTASK_RETRIES times.
            if (
                not validation_result["success"]
                and validation_result.get("missing_files")
                and not validation_result.get("invalid_paths")
            ):
                repair = repair_subtask_files(
                    next_subtask, validation_result["missing_files"], spec_dir
                )
                if repair["repaired"]:
                    moved_str = ", ".join(repair["moved"])
                    print_status(
                        f"Auto-repaired plan: reclassified {len(repair['moved'])} "
                        f"missing file(s) as files_to_create ({moved_str})",
                        "success",
                    )
                    if repair.get("error"):
                        print(muted(f"Note: {repair['error']}"))
                    if task_logger:
                        task_logger.log_info(
                            f"Plan auto-repair: moved {moved_str} from files_to_modify to files_to_create",
                            LogPhase.CODING,
                        )
                    # Re-validate now that the misclassification is fixed
                    validation_result = validate_subtask_files(
                        next_subtask, project_dir
                    )
