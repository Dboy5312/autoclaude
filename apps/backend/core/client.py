...
def create_client(
    ...
    # AutoClaude personalization preamble: output style + persona (SOUL.md) + enabled skills.
    # Sent from the Electron main process via AUTOCLAUDE_AGENT_PREAMBLE so the model sees
    # user-configured shape/tone/capability hints alongside the base prompt.
    preamble = os.environ.get("AUTOCLAUDE_AGENT_PREAMBLE", "")
    if preamble:
        if is_windows():
            preamble_budget = (
                WINDOWS_MAX_SYSTEM_PROMPT_CHARS
            ...
            if 0 < preamble_budget < len(preamble):
                preamble = preamble[:preamble_budget]
                print("   - AutoClaude preamble: truncated (Windows command-line limit)")
        base_prompt = f"{base_prompt}\n\n{preamble}"
        print("   - AutoClaude preamble: included (output-style + persona + skills)")
    print()
