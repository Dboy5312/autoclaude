...
/** Generic file attachment supporting any non-image file the user drops onto a
 * task. PDFs and binaries travel as base64; readable text files travel as
 * `textContent`. After persistence, only `path` is kept (data is dropped to
 * keep the task JSON small). */
export type FileAttachmentKind = 'pdf' | 'text' | 'binary';

export interface FileAttachment {
  id: string;
  filename: string;
  mimeType: string;     // e.g., 'application/pdf' or 'text/markdown'
  size: number;
  kind: FileAttachmentKind;
  /** Base64 (PDFs / binaries) — present pre-save, dropped post-save. */
  data?: string;
  /** UTF-8 text content (text kind) — present pre-save, dropped post-save. */
  textContent?: string;
  /** Relative path under the spec dir after the file has been written to disk. */
  path?: string;
}
...
// ModelType aliases ModelTypeShort so adding new model shorthands in settings.ts
// flows through every consumer (TaskMetadata, SpecCreationMetadata, etc.) without
// the two unions drifting apart.
...
export interface TaskMetadata {
  ...
  // Non-image attachments — PDFs, text files, code, configs, etc.
