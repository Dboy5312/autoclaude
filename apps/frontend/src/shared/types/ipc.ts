...
export interface ElectronAPI {
  ...
  listRenderableFiles: (projectPath: string) => Promise<IPCResult<import('./preview').RenderableFile[]>>;
  ...
  // Hooks system (lifecycle event hooks: bash command / http webhook / prompt)
  hooks: import('../../preload/api/hooks-api').HooksAPI['hooks'];

  // Scheduled tasks (cron) — create / list / delete cron schedules
  cron: import('../../preload/api/cron-api').CronAPI['cron'];

  // Plugin loader — discover and toggle plugins under <userData>/plugins
  plugins: import('../../preload/api/plugin-api').PluginAPI['plugins'];

  // Skills runtime — capability packs under <userData>/skills
  skills: import('../../preload/api/skills-api').SkillsExtAPI['skills'];

  // Output styles — response format presets under <userData>/output-styles
  outputStyles: import('../../preload/api/skills-api').SkillsExtAPI['outputStyles'];

  // Persona / SOUL.md — insights-chat output style + persona
  persona: import('../../preload/api/skills-api').SkillsExtAPI['persona'];

  // Global agent instructions — AGENT_INSTRUCTIONS.md, prepended to pipeline phases
  agentInstructions: import('../../preload/api/skills-api').SkillsExtAPI['agentInstructions'];
}
