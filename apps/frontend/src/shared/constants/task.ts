...
// Maximum non-image file size (10 MB). PDFs hit the same cap; text files
// shouldn't be enormous since they consume tokens.
export const MAX_FILE_SIZE = 10 * 1024 * 1024;

// Maximum number of non-image attachments per task
export const MAX_FILES_PER_TASK = 20;

// PDFs are sent as base64 document blocks. Multiple types accepted for
// flexibility — Anthropic accepts application/pdf.
export const ALLOWED_PDF_MIME_TYPES = ['application/pdf'] as const;
export const ALLOWED_PDF_EXTENSIONS = ['.pdf'] as const;

// Text-readable file extensions — read as UTF-8 and inlined into the agent's
// context as the file content. Any text-like file works; binary garbage gets
// rejected at upload time by reading the first kilobyte and looking for NULs.
export const ALLOWED_TEXT_EXTENSIONS = [
  ...
  '.cs', '.fs', '.swift', '.m', '.mm', '.php', '.lua', '.dart',
  '.sh', '.bash', '.zsh', '.fish', '.ps1', '.psm1', '.bat', '.cmd',
  '.sql', '.graphql', '.gql', '.proto',
  '.dockerfile', '.gitignore', '.gitattributes', '.editorconfig',
] as const;

export const ALLOWED_FILE_TYPES_DISPLAY = 'PDFs, text, markdown, JSON, YAML, CSV, source code (most languages), configs';
