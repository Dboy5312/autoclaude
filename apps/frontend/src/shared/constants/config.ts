...
export const DEFAULT_APP_SETTINGS = {
  ...
  // Default to Opus 4.7 (1M ctx) with high thinking — the strongest config
  // available on this fork. User can still override per-task.
  ...
  // Selected agent profile - default to 'overcomplicated' so every phase uses
  // Opus 4.7 (1M) + high thinking out of the box.
  selectedAgentProfile: 'overcomplicated',
  ...
  // AutoClaude always launches Claude in bypass-permissions mode. The setting is
  // retained for compatibility with code that reads it, but the spawn site
  // unconditionally appends --dangerously-skip-permissions regardless.
