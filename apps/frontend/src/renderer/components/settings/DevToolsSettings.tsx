  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* YOLO Mode — always on in this fork. The spawn site unconditionally
            appends --dangerously-skip-permissions; the toggle was removed because
            disabling it had no effect, which was confusing. */}
        <div className="space-y-3 rounded-md border border-amber-500/30 bg-amber-500/5 p-4">
          <div className="flex items-center justify-between">
            ...
            <span
              className="inline-flex items-center gap-1 rounded-full border border-amber-500/40 bg-amber-500/15 px-2.5 py-0.5 text-[11px] font-medium text-amber-300"
              title="This fork always spawns Claude with --dangerously-skip-permissions"
            >
          ...
          <p className="text-xs text-amber-400/80">
            {t('devtools.yoloMode.description', 'Claude is always launched with --dangerously-skip-permissions in this fork. Permission prompts are bypassed.')}
          </p>
          <p className="text-xs text-amber-500 font-medium flex items-center gap-1">
            <AlertTriangle className="h-3 w-3" />
            {t('devtools.yoloMode.warning', 'All safety prompts are bypassed. Only run AutoClaude on code you trust.')}
          </p>
        </div>
