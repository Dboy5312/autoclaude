...
import { Brain, Scale, Zap, Check, Sparkles, ChevronDown, ChevronUp, RotateCcw, Settings2, Flame, Atom } from 'lucide-react';
...
export function AgentProfileSettings() {
  ...
  const handleSelectProfile = async (profileId: string) => {
    ...
    // When selecting a preset, reset to that preset's defaults. Also push through
    // the profile's optional feature/default-model overrides (e.g. OVERCOMPLICATED)
    // so every AI-using setting matches the profile, not just phase configuration.
    ...
    if (profile.featureModels) update.featureModels = profile.featureModels;
    if (profile.featureThinking) update.featureThinking = profile.featureThinking;
    if (profile.defaultModel) update.defaultModel = profile.defaultModel;

    const success = await saveSettings(update);
    if (!success) {
