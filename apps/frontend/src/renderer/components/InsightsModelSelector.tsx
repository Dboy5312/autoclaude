...
import { Brain, Scale, Zap, Sparkles, Sliders, Check, Flame } from 'lucide-react';
...
export function InsightsModelSelector({
  ...
  // Get the appropriate icon. Fall back to Scale if a profile references an
  // icon name we don't have in the map (prevents `<undefined />` crashes when
  // new profiles ship with new icon names).
  const Icon = selectedProfileId === 'custom'
    ? Sliders
    : (profile?.icon ? (iconMap[profile.icon] ?? Scale) : Scale);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-64">
          ...
          {DEFAULT_AGENT_PROFILES.filter(p => !p.isAutoProfile).map((p) => {
            const ProfileIcon = iconMap[p.icon || 'Brain'] ?? Brain;
