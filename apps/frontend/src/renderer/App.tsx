...
import { InsightsWithPreview } from './components/preview/InsightsWithPreview';
import { KanbanWithPreview } from './components/preview/KanbanWithPreview';
import { ScheduledTasks } from './components/ScheduledTasks';
import { ConnectorsCatalog } from './components/ConnectorsCatalog';
import { Plugins } from './components/Plugins';
import { Skills } from './components/Skills';
import { Buildbay } from './components/Buildbay';
...
import { useTaskStore, loadTasks, createTask, startTaskOrQueue } from './stores/task-store';
...
export function App() {
  ...
  // Listen for scheduled-task fires from the main-process ScheduledTaskManager.
  // The manager keeps the schedule; we create the actual task and (optionally)
  // kick the agent off here so the existing task pipeline owns everything.
  useEffect(() => {
    const unsub = window.electronAPI.cron.onFired(async (payload) => {
      const title = `${payload.scheduledTaskName} — ${new Date(payload.firedAt).toLocaleString()}`;
      const task = await createTask(payload.projectId, title, payload.prompt);
      if (task && payload.autoStart) {
        try {
          await startTaskOrQueue(task.id);
        } catch (e) {
          console.error('[cron] auto-start failed:', e);
        }
      }
    });
    return () => unsub();
  ...
  const handleRefreshTasks = async () => {
    ...
    // Hold the spinner for at least 400ms so the user sees that something
    // happened — the actual reload often completes faster than the eye can track.
    ...
    try {
      ...
      await Promise.all([
        loadTasks(currentProjectId, { forceRefresh: true }),
        minDuration,
      ]);
    } finally {
            ...
            {selectedProject ? (
              <>
                ...
                {activeView === 'insights' && (activeProjectId || selectedProjectId) && (
                  <InsightsWithPreview projectId={activeProjectId || selectedProjectId!} />
                )}
                ...
                {activeView === 'hooks' && (
                  <Hooks projectPath={selectedProject?.path ?? null} />
                )}
                {activeView === 'scheduled' && <ScheduledTasks />}
                {activeView === 'connectors' && <ConnectorsCatalog />}
                {activeView === 'plugins' && <Plugins />}
                {activeView === 'skills' && <Skills />}
                {activeView === 'buildbay' && <Buildbay />}
              </>
            ) : (
              <WelcomeScreen
                projects={projects}
