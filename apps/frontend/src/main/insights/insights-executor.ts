...
import { buildChatPreamble } from '../agent/preamble-builder';
...
export class InsightsExecutor extends EventEmitter {
  ...
  async execute(
    ...
    // Chat preamble (output style + SOUL.md persona). insights_runner.py picks
    // this up and appends to its system prompt. Pipeline phases never see it.
    const chatPreambleEnv: Record<string, string> = {};
    try {
      const chatPreamble = buildChatPreamble();
      if (chatPreamble) chatPreambleEnv['AUTOCLAUDE_CHAT_PREAMBLE'] = chatPreamble;
    } catch (e) {
      console.error('[Insights] Failed to build chat preamble:', e);
    }

    // Spawn Python process
    const proc = spawn(this.config.getPythonPath(), args, {
      cwd: autoBuildSource,
      env: { ...processEnv, ...chatPreambleEnv }
    });
