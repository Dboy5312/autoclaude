...
import { fireHooks } from "../hooks/hooks-runner";
import { watchProjectHooks } from "./hooks-handlers";
import type { HookEvent, HookInput } from "../../shared/types/hooks";

/** Map ExecutionPhase string -> hook event names. Returns null for phases
 * that don't have a corresponding hook event (e.g. rate_limit_paused). */
function phaseHookEvents(phase: string | undefined): { pre: HookEvent; post: HookEvent } | null {
  switch (phase) {
    case 'planning': return { pre: 'PrePhasePlanning', post: 'PostPhasePlanning' };
    case 'coding': return { pre: 'PrePhaseCoding', post: 'PostPhaseCoding' };
    case 'qa_review':
    case 'qa_fixing':
      return { pre: 'PrePhaseValidation', post: 'PostPhaseValidation' };
    default: return null;
  }
}

/** Last-seen phase per task. Lets us detect transitions and fire pre/post pairs. */
const lastPhaseByTaskId = new Map<string, string>();
...
export function registerAgenteventsHandlers(
  ...
  agentManager.on("execution-progress", (taskId: string, progress: ExecutionProgressData, projectId?: string) => {
    ...
    // ============ Hook firing on phase transitions ============
    // Fire post-hook for the phase we're leaving and pre-hook for the one
    // we're entering. Hook execution is fire-and-forget; we don't await so
    // the renderer message above isn't blocked. Blocking-hook semantics
    // (status='blocked') currently log only — full enforcement requires
    // pausing the python subprocess which is v2 work.
    if (project && progress.phase) {
      const last = lastPhaseByTaskId.get(taskId);
      if (last !== progress.phase) {
        // Make sure the project's hooks file is being watched
        watchProjectHooks(project.path);

        const buildInput = (event: HookEvent, status: 'started' | 'completed'): HookInput => ({
        ...
        const oldEvents = phaseHookEvents(last);
        if (oldEvents) {
          fireHooks(oldEvents.post, buildInput(oldEvents.post, 'completed'), project.path)
            .catch((err) => console.warn('[Hooks] post-phase fire failed:', err));
        }
        const newEvents = phaseHookEvents(progress.phase);
        if (newEvents) {
          fireHooks(newEvents.pre, buildInput(newEvents.pre, 'started'), project.path)
            .catch((err) => console.warn('[Hooks] pre-phase fire failed:', err));
        }
        lastPhaseByTaskId.set(taskId, progress.phase);
      }
    }
  });
