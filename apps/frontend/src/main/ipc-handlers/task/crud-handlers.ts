...
export function registerTaskCRUDHandlers(agentManager: AgentManager): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // Process and save generic file attachments (PDFs, text files, configs).
      // Same attachments/ dir, no MIME-type allowlist — frontend already
      // classifies + validates extensions. We just persist what we received.
      if (taskMetadata.attachedFiles && taskMetadata.attachedFiles.length > 0) {
        const attachmentsDir = path.join(specDir, 'attachments');
        mkdirSync(attachmentsDir, { recursive: true });
        const resolvedAttachmentsDir = path.resolve(attachmentsDir);

        const savedFiles: typeof taskMetadata.attachedFiles = [];
        const usedNames = new Set<string>(
          (taskMetadata.attachedImages ?? []).map((i) => i.filename),
        );

        for (const file of taskMetadata.attachedFiles) {
          const baseName = path.basename(file.filename);
          if (!baseName || baseName === '.' || baseName === '..') {
            console.warn(`[TASK_CREATE] Skipping file with invalid name: ${file.filename}`);
            continue;
          }
          // Avoid collision with image filenames (defensive — frontend uses different ids)
          ...
          const filePath = path.join(attachmentsDir, finalName);
          const resolvedPath = path.resolve(filePath);
          if (!resolvedPath.startsWith(resolvedAttachmentsDir + path.sep)) {
            console.warn(`[TASK_CREATE] Skipping file with path traversal attempt: ${file.filename}`);
            continue;
          }

          try {
            if (file.kind === 'text' && typeof file.textContent === 'string') {
              writeFileSync(filePath, file.textContent, 'utf-8');
            } else if (file.data) {
              writeFileSync(filePath, Buffer.from(file.data, 'base64'));
            } else {
              console.warn(`[TASK_CREATE] Skipping file with no data/textContent: ${file.filename}`);
              continue;
            }
            savedFiles.push({
              id: file.id,
              ...
              // Drop data/textContent to keep the persisted task small
            });
          } catch (err) {
            console.error(`Failed to save file ${finalName}:`, err);
          }
        }

        taskMetadata.attachedFiles = savedFiles;
      }

      // Append a footer to the description listing all attached files (images +
      // generic files) so the spec_writer / coder phases surface them in spec.md
      // and the agent reads them via standard file ops.
      const attachmentRefs: string[] = [];
      for (const img of taskMetadata.attachedImages ?? []) {
        if (img.path) attachmentRefs.push(`- ${img.path} (image, ${img.mimeType})`);
      }
      for (const f of taskMetadata.attachedFiles ?? []) {
        if (f.path) attachmentRefs.push(`- ${f.path} (${f.kind}, ${f.mimeType})`);
      }
      const enrichedDescription = attachmentRefs.length > 0
        ? `${description}\n\n## Attached Files\n\nThe user attached the following files. **Read them before planning** — they contain context, requirements, designs, or data relevant to this task.\n\n${attachmentRefs.join('\n')}\n`
        : description;
