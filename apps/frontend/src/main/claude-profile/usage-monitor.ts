...
export class UsageMonitor extends EventEmitter {
  ...
  // Global rate-limit circuit breaker. When the usage endpoint returns 429,
  // ALL polling pauses until this timestamp (honoring Retry-After) instead of
  // hammering the endpoint every interval and flooding the log with errors.
  private rateLimitPausedUntil = 0;
  private static RATE_LIMIT_MIN_PAUSE_MS = 60 * 1000; // floor for the pause window
  // Collapse repeated identical API-error logs into a counted summary.
  private apiErrorLogCount = 0;
  private lastApiErrorLogAt = 0;
  private static API_ERROR_LOG_THROTTLE_MS = 60 * 1000;
  ...
  /**
   * Log usage-API errors at most once per throttle window, with a rolled-up
   * count of suppressed occurrences. Replaces the previous unconditional
   * console.error that flooded the log on every 429.
   */
  private logApiErrorThrottled(
    status: number,
    provider: string,
    endpoint: string,
    pauseMs?: number
  ): void {
    this.apiErrorLogCount++;
    const now = Date.now();
    if (now - this.lastApiErrorLogAt < UsageMonitor.API_ERROR_LOG_THROTTLE_MS) {
      return; // within throttle window — suppress, just count
    }
    const suppressed = this.apiErrorLogCount - 1;
    this.lastApiErrorLogAt = now;
    this.apiErrorLogCount = 0;
    const pauseNote = pauseMs ? ` — pausing usage polling for ${Math.round(pauseMs / 1000)}s` : '';
    const suppressedNote = suppressed > 0 ? ` (${suppressed} similar suppressed)` : '';
    console.warn(
      `[UsageMonitor] API error ${status} from ${provider}${pauseNote}${suppressedNote}`,
      this.isDebug ? { endpoint } : ''
    );
  }
  ...
  start(): void {
    ...
    // 60s base (was 30s) reduces baseline call volume that was tripping 429s.
    const baseInterval = settings.usageCheckInterval || 60000;

    this.debugLog('[UsageMonitor] Starting with base interval: ' + baseInterval + ' ms (+jitter)');

    // Check immediately
    this.checkUsageAndSwap();

    // Then check periodically, with jitter so multiple windows/instances don't
    // align their polls and burst the endpoint simultaneously.
    const jitter = Math.floor(baseInterval * 0.2);
    this.intervalId = setInterval(() => {
      this.checkUsageAndSwap();
    }, baseInterval + Math.floor(Math.random() * jitter));
  }
  ...
  private async checkUsageAndSwap(): Promise<void> {
    ...
    // Circuit breaker: if the usage endpoint recently 429'd, skip this whole
    // cycle until the pause window elapses rather than re-hitting it.
  ...
  private async fetchUsageViaAPI(
    ...
    try {
      ...
      if (!response.ok) {
        // 429: trip the global circuit breaker, honor Retry-After, and stop the
        // log flood. Previously this logged console.error every 30s with no pause.
        if (response.status === 429) {
          const retryAfterRaw = response.headers.get('retry-after');
          const retryAfterMs = retryAfterRaw
            ? (Number.isNaN(Number(retryAfterRaw))
                ? Math.max(0, Date.parse(retryAfterRaw) - Date.now())
                : Number(retryAfterRaw) * 1000)
            : 0;
          const pauseMs = Math.max(retryAfterMs, UsageMonitor.RATE_LIMIT_MIN_PAUSE_MS);
          this.rateLimitPausedUntil = Date.now() + pauseMs;
          this.apiFailureTimestamps.set(profileId, Date.now());
          this.logApiErrorThrottled(429, provider, usageEndpoint, pauseMs);
          return null;
        }

        this.logApiErrorThrottled(response.status, provider, usageEndpoint);
