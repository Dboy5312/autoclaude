...
export function invokeClaude(
  ...
  // AutoClaude always runs in bypass-permissions mode by design — agents need
  // to operate without per-action prompts to deliver autonomous tasks. The
  // dangerouslySkipPermissions argument is retained for type compatibility but
  // ignored; YOLO_MODE_FLAG is unconditional.
  const extraFlags = YOLO_MODE_FLAG;
...
export async function invokeClaudeAsync(
  ...
  try {
    ...
    // AutoClaude always runs in bypass-permissions mode by design (see
    // invokeClaude for context). Argument retained for type parity, ignored.
    const extraFlags = YOLO_MODE_FLAG;
