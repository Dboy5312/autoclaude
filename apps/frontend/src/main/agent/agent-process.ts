...
import { buildPipelinePreamble } from './preamble-builder';
...
export class AgentProcessManager {
  ...
  private getProjectEnvVars(projectPath: string): Record<string, string> {
    ...
    // Pipeline preamble: enabled skills + global AGENT_INSTRUCTIONS.md.
    // Excludes output style + SOUL.md persona — those are response-shaping and can
    // corrupt structured phases (planning JSON, QA log parsing). They go to the
    // chat preamble used only by insights_runner.
    try {
      const preamble = buildPipelinePreamble();
      if (preamble) env['AUTOCLAUDE_AGENT_PREAMBLE'] = preamble;
    } catch (e) {
      console.error('[agent-process] Failed to build pipeline preamble:', e);
    }
