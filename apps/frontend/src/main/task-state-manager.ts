...
import { watchProjectHooks } from './ipc-handlers/hooks-handlers';
...
export class TaskStateManager {
  ...
  private getOrCreateActor(taskId: string): TaskActor {
    ...
    actor.subscribe((snapshot) => {
      ...
      // Fire task-lifecycle hooks. Status transitions to a stable lifecycle
      // event are mapped here. Fire-and-forget; hook failures don't affect state.
      this.maybeFireLifecycleHook(taskId, task, project, status, lastState, stateValue);
    });
  ...
  /**
   * Fire task-lifecycle hooks on state transitions. Maps:
   *   <any> -> 'planning'/'coding' (first time)  -> TaskStarted
   *   <any> -> 'done'                            -> TaskCompleted
   *   <any> -> 'error' (when reviewReason='stuck') -> TaskStuck
   *
   * Fire-and-forget; failure logs but doesn't disturb the state machine.
   */
  private maybeFireLifecycleHook(
    taskId: string,
    task: Task,
    project: Project,
    status: TaskStatus,
    lastXState: string | undefined,
    newXState: string,
  ): void {
    ...
    if (status === 'done') {
      ...
      (newXState === 'planning' || newXState === 'coding') &&
      (!lastXState || lastXState === 'idle' || lastXState === 'pending')
    ) {
      event = 'TaskStarted';
    } else if (status === 'error') {
      // Only fire TaskStuck once per task per error transition.
      event = 'TaskStuck';
    }
    if (!event) return;
    ...
    fireHooks(event, input, project.path).catch((err) =>
      console.warn('[Hooks] lifecycle fire failed:', err),
    );
  }
